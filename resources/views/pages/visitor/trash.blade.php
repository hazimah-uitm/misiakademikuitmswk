        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('user') }}"></i>Senarai Data Pengunjung</a>
                    </li>
                    <li class="breadcrumb-item active" aria-current="page">Senarai Data Pengunjung Dipadam</li>
                </ol>
            </nav>
        </div>
    </div>
    <!--end breadcrumb-->
    <h6 class="mb-0 text-uppercase">Senarai Data Pengunjung Dipadam</h6>
    <hr />
    <div class="card">
        <div class="card-body">
                ...
                <table class="table">
                    ...
                    <tbody>
                        @if (count($trashList) > 0)
                            @foreach ($trashList as $visitor)
                                <tr>
                                    <td>{{ ucfirst($visitor->full_name) }}</td>
                                    <td>{{ $visitor->email ?: '-' }}</td>
                                    <td>{{ $visitor->lokasi ?: '-' }}</td>
                                    <td>
                                        <a href="{{ route('visitor.restore', $visitor->id) }}"
            ...
            <div class="mt-3 d-flex justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <form action="{{ route('visitor') }}" method="GET" id="perPageForm">
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        @isset($user)
                            <form class="d-inline" method="POST" action="{{ route('visitor.forceDelete', $user->id) }}">
                                {{ method_field('delete') }}
                                {{ csrf_field() }}
